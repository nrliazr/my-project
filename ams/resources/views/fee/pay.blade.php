    ...
    <div class="bg-white shadow rounded-lg min-w-fit min-h-full mx-4 p-4 sm:py-8 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="/fee" enctype="multipart/form-data">
            ...
            <div class="m-4 w-1/2">
                ...
                <select id="payment_method" onchange="showBankOptions(this.value)"
                    class="mb-2 bg-white border-1 border-indigo-300 input-text text-base rounded-lg placeholder:text-indigo-900 focus:ring-indigo-400 focus:border-indigo-400 block w-full p-2.5"
                    type="text" name="payment_method" autocomplete="payment_method">
                    <option value="Online payment">Online payment</option>
                </select>

            </div>
            <div class="flex items-center justify-end gap-4 pt-4">
                <a href="{{ route('fee.show', $student->id) }}"
...
<script>
    function showBankOptions(value) {
    ...
    function donePayment() {
        ...
        formData.append('selected_month', document.getElementById("selected_month").value);
        fetch('/fee', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                // Redirect the user back to the show template page
